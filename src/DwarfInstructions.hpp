...
///
template <typename A> struct CFI_Reference
{
...
template <typename A> struct CFI_Atom_Info
{
    typedef typename A::pint_t pint_t;
    pint_t address;
    uint32_t size;
...
template <typename A, typename R> class DwarfInstructions
{
  public:
    ...
    static const char* parseCFIs(A& addressSpace, pint_t ehSectionStart, uint32_t sectionLength, CFI_Atom_Info<A>* infos, uint32_t infosCount, void* ref, WarnFunc warn);

    static compact_unwind_encoding_t createCompactEncodingFromFDE(A& addressSpace, pint_t fdeStart, pint_t* lsda, pint_t* personality, char warningBuffer[1024]);
  ...
  private:
    ...
    static pint_t getSavedRegister(A& addressSpace, const R& registers, pint_t cfa, const typename CFI_Parser<A>::RegisterLocation& savedReg);
    static double getSavedFloatRegister(A& addressSpace, const R& registers, pint_t cfa, const typename CFI_Parser<A>::RegisterLocation& savedReg);
    static v128 getSavedVectorRegister(A& addressSpace, const R& registers, pint_t cfa, const typename CFI_Parser<A>::RegisterLocation& savedReg);
    ...
    static compact_unwind_encoding_t
    createCompactEncodingFromProlog(A& addressSpace, pint_t funcAddr, const Registers_x86&, const typename CFI_Parser<A>::PrologInfo& prolog, char warningBuffer[1024]);
    ...
    static compact_unwind_encoding_t
    createCompactEncodingFromProlog(A& addressSpace, pint_t funcAddr, const Registers_x86_64&, const typename CFI_Parser<A>::PrologInfo& prolog, char warningBuffer[1024]);
    ...
    static compact_unwind_encoding_t
    createCompactEncodingFromProlog(A& addressSpace, pint_t funcAddr, const Registers_ppc&, const typename CFI_Parser<A>::PrologInfo& prolog, char warningBuffer[1024]);
};

template <typename A, typename R>
const char* DwarfInstructions<A, R>::parseCFIs(A& addressSpace, pint_t ehSectionStart, uint32_t sectionLength, CFI_Atom_Info<A>* infos, uint32_t infosCount, void* ref, WarnFunc warn)
{
...
template <typename A, typename R>
compact_unwind_encoding_t DwarfInstructions<A, R>::createCompactEncodingFromFDE(A& addressSpace, pint_t fdeStart, pint_t* lsda, pint_t* personality, char warningBuffer[1024])
{
    typename CFI_Parser<A>::FDE_Info fdeInfo;
    typename CFI_Parser<A>::CIE_Info cieInfo;
    R dummy; // for proper selection of architecture specific functions
...
template <typename A, typename R>
typename A::pint_t DwarfInstructions<A, R>::getSavedRegister(A& addressSpace, const R& registers, pint_t cfa, const typename CFI_Parser<A>::RegisterLocation& savedReg)
{
...
template <typename A, typename R> double DwarfInstructions<A, R>::getSavedFloatRegister(A& addressSpace, const R& registers, pint_t cfa, const typename CFI_Parser<A>::RegisterLocation& savedReg)
{
    switch (savedReg.location)
    {
        case CFI_Parser<A>::kRegisterInCFA:
...
template <typename A, typename R> v128 DwarfInstructions<A, R>::getSavedVectorRegister(A& addressSpace, const R& registers, pint_t cfa, const typename CFI_Parser<A>::RegisterLocation& savedReg)
{
    switch (savedReg.location)
    {
        case CFI_Parser<A>::kRegisterInCFA:
...
template <typename A, typename R> int DwarfInstructions<A, R>::lastRestoreReg(const Registers_x86_64&)
{
    static_assert((int) CFI_Parser<A>::kMaxRegisterNumber > (int) DW_X86_64_RET_ADDR, "(int)CFI_Parser<A>::kMaxRegisterNumber MUST be > (int)DW_X86_64_RET_ADDR");
    return DW_X86_64_RET_ADDR;
}

template <typename A, typename R> bool DwarfInstructions<A, R>::isReturnAddressRegister(int regNum, const Registers_x86_64&)
{
    return (regNum == DW_X86_64_RET_ADDR);
}

template <typename A, typename R> typename A::pint_t DwarfInstructions<A, R>::getCFA(A& addressSpace, const typename CFI_Parser<A>::PrologInfo& prolog, const Registers_x86_64& registers)
{
...
template <typename A, typename R> compact_unwind_encoding_t DwarfInstructions<A, R>::encodeToUseDwarf(const Registers_x86_64&)
{
    return UNWIND_X86_64_MODE_DWARF;
}

template <typename A, typename R> compact_unwind_encoding_t DwarfInstructions<A, R>::encodeToUseDwarf(const Registers_x86&)
{
    return UNWIND_X86_MODE_DWARF;
}

template <typename A, typename R> uint32_t DwarfInstructions<A, R>::getRBPEncodedRegister(uint32_t reg, int32_t regOffsetFromBaseOffset, bool& failure)
{
...
DwarfInstructions<A, R>::createCompactEncodingFromProlog(A& addressSpace, pint_t funcAddr, const Registers_x86_64& r, const typename CFI_Parser<A>::PrologInfo& prolog, char warningBuffer[1024])
{
    warningBuffer[0] = '\0';
    ...
    bool standardRBPframe = ((prolog.cfaRegister == UNW_X86_64_RBP) && (prolog.cfaRegisterOffset == 16) && (prolog.savedRegisters[UNW_X86_64_RBP].location == CFI_Parser<A>::kRegisterInCFA) &&
...
//
template <typename A, typename R> int DwarfInstructions<A, R>::lastRestoreReg(const Registers_x86&)
{
    static_assert((int) CFI_Parser<A>::kMaxRegisterNumber > (int) DW_X86_RET_ADDR, "(int)CFI_Parser<A>::kMaxRegisterNumber MUST be > (int)DW_X86_RET_ADDR");
    return DW_X86_RET_ADDR;
}

template <typename A, typename R> bool DwarfInstructions<A, R>::isReturnAddressRegister(int regNum, const Registers_x86&)
{
    return (regNum == DW_X86_RET_ADDR);
}

template <typename A, typename R> typename A::pint_t DwarfInstructions<A, R>::getCFA(A& addressSpace, const typename CFI_Parser<A>::PrologInfo& prolog, const Registers_x86& registers)
{
    if (prolog.cfaRegister != 0)
        return registers.getRegister(prolog.cfaRegister) + prolog.cfaRegisterOffset;
    else if (prolog.cfaExpression != 0)
...
template <typename A, typename R> uint32_t DwarfInstructions<A, R>::getEBPEncodedRegister(uint32_t reg, int32_t regOffsetFromBaseOffset, bool& failure)
{
...
template <typename A, typename R>
compact_unwind_encoding_t
DwarfInstructions<A, R>::createCompactEncodingFromProlog(A& addressSpace, pint_t funcAddr, const Registers_x86& r, const typename CFI_Parser<A>::PrologInfo& prolog, char warningBuffer[1024])
{
    warningBuffer[0] = '\0';
    ...
    // figure out which kind of frame this function uses
    bool standardEBPframe = ((prolog.cfaRegister == UNW_X86_EBP) && (prolog.cfaRegisterOffset == 8) && (prolog.savedRegisters[UNW_X86_EBP].location == CFI_Parser<A>::kRegisterInCFA) &&
...
//
template <typename A, typename R> int DwarfInstructions<A, R>::lastRestoreReg(const Registers_ppc&)
{
    static_assert((int) CFI_Parser<A>::kMaxRegisterNumber > (int) UNW_PPC_SPEFSCR, "(int)CFI_Parser<A>::kMaxRegisterNumber MUST be > (int)UNW_PPC_SPEFSCR");
...
template <typename A, typename R> typename A::pint_t DwarfInstructions<A, R>::getCFA(A& addressSpace, const typename CFI_Parser<A>::PrologInfo& prolog, const Registers_ppc& registers)
{
    if (prolog.cfaRegister != 0)
        return registers.getRegister(prolog.cfaRegister) + prolog.cfaRegisterOffset;
    else if (prolog.cfaExpression != 0)
...
template <typename A, typename R> compact_unwind_encoding_t DwarfInstructions<A, R>::encodeToUseDwarf(const Registers_ppc&)
{
    return UNWIND_X86_MODE_DWARF;
}
...
template <typename A, typename R>
compact_unwind_encoding_t
DwarfInstructions<A, R>::createCompactEncodingFromProlog(A& addressSpace, pint_t funcAddr, const Registers_ppc& r, const typename CFI_Parser<A>::PrologInfo& prolog, char warningBuffer[1024])
{
    warningBuffer[0] = '\0';
    return UNWIND_X86_MODE_DWARF;
}
