...
template <typename A>
int CompactUnwinder_x86<A>::stepWithCompactEncodingFrameless(compact_unwind_encoding_t encoding, uint32_t functionStart, A& addressSpace, Registers_x86& registers, bool indirectStackSize)
{
...
#if SUPPORT_OLD_BINARIES
template <typename A> int CompactUnwinder_x86<A>::stepWithCompactEncodingCompat(compact_unwind_encoding_t compactEncoding, uint32_t functionStart, A& addressSpace, Registers_x86& registers)
{
    // fprintf(stderr, "stepWithCompactEncoding(0x%08X)\n", compactEncoding);
...
template <typename A> int CompactUnwinder_x86_64<A>::stepWithCompactEncodingRBPFrame(compact_unwind_encoding_t compactEncoding, uint64_t functionStart, A& addressSpace, Registers_x86_64& registers)
{
    ...
    if (b == 0xC5)
    {
        if ((b = addressSpace.get8(registers.getIP() + 1)) == 0xF8 && (b = addressSpace.get8(registers.getIP() + 2)) == 0x77)
        {
...
template <typename A>
int CompactUnwinder_x86_64<A>::stepWithCompactEncodingFrameless(compact_unwind_encoding_t encoding, uint64_t functionStart, A& addressSpace, Registers_x86_64& registers, bool indirectStackSize)
{
    ...
    if (regCount > 0)
    {
        for (int8_t i = regCount - 1; i >= 0; --i)
        {
            ...
            if ((UNWIND_X86_64_REG_RBP == registersSaved[i]) || (UNWIND_X86_64_REG_RBX == registersSaved[i]))
                curAddr += 1;
            else
                curAddr += 2;
        }
...
#if SUPPORT_OLD_BINARIES
template <typename A> int CompactUnwinder_x86_64<A>::stepWithCompactEncodingCompat(compact_unwind_encoding_t compactEncoding, uint64_t functionStart, A& addressSpace, Registers_x86_64& registers)
{
