...
class BigEndian
{
  public:
    ...
    static uint32_t getBits(const uint32_t& from, uint8_t firstBit, uint8_t bitCount) INLINE
    {
        return getBitsRaw(get32(from), firstBit, bitCount);
    }
    static void setBits(uint32_t& into, uint32_t value, uint8_t firstBit, uint8_t bitCount) INLINE
    {
    ...
    static void setBitsRaw(uint32_t& into, uint32_t value, uint8_t firstBit, uint8_t bitCount) INLINE
    {
        uint32_t temp = into;
...
template <typename _E> class Pointer32
{
  public:
    typedef uint32_t uint_t;
    typedef _E E;

    static uint64_t getP(const uint_t& from) INLINE
    {
...
template <typename _E> class Pointer64
{
  public:
